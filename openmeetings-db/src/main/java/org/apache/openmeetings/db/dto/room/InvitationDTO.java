...
public class InvitationDTO implements Serializable {
	...
	public static List<InvitationDTO> list(List<Invitation> l) {
		List<InvitationDTO> iList = new ArrayList<>();
		if (l != null) {
			for (Invitation i : l) {
				iList.add(new InvitationDTO(i));
	...
	public InvitationDTO(Invitation i) {
		this.roomId = i.getRoom().getId();
		this.roomComment = i.getRoom().getComment();
		this.roomName = i.getRoom().getName();
		this.groupId = i.getRoom().getGroups().get(0).getId();
		this.invitedBy = i.getInvitedBy().getId();
		this.invitee = i.getInvitee().getId();
		this.validFrom = i.getValidFrom().toString();
		this.validTo = i.getValidTo().toString();
		this.valid = i.getValid();
	}
