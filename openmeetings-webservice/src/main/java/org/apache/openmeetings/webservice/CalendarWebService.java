...
public class CalendarWebService extends BaseWebService {
	...
	@GET
	@Path("/group/{id}")
	public List<AppointmentDTO> getByGroup(@QueryParam("sid") @WebParam(name="sid") String sid
			, @PathParam("id") @WebParam(name="id") Long id
	)
	{
		log.debug("get by group id :", id);
		return performCall(sid, Right.ROOM
				, sd -> AppointmentDTO.list(dao.getByGroup(id)));
	}
